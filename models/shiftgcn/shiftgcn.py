...
import sys
sys.path.append("./models/shiftgcn/Temporal_shift/")
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # N*M,C,T,V
        c_new = x.size(1)
        # feature = x.view(N, M, c_new, -1, V).mean(4).permute(0,1,3,2).contiguous()
        feature = x.view(N, M, c_new, -1, V).mean(1)  # n c t v
...
class ShiftGCNExtractor(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3):
        super(ShiftGCNExtractor, self).__init__()
        # pretraining model
    ...
    def forward(self, x):
        pooling_feature = self.feature_extractor(x)  # n, 256    n, 256, 16, 25
